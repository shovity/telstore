...
async function roundTrip({ label, size, chunkSize }) {
  ...
  // Back in this suite as of 2026-09-08. It was dropped on the day this file was written,
  // because `list` searched Telegram's text index for "#telstore" and that index answered
  // nothing at all about a channel that had just been created — for a whole day, while the
  // same chat handed over every one of its documents on request. `list` walks the chat now.
...
test('a backup cut into chunks below the big-file threshold survives a round trip', { skip, timeout: 600_000 }, async () => {
  ...
  // The other half of what a fake cannot say: what `offsetId` means to the real server. The
  // walk `list` is built on pages by it, and a fake client would agree with any spelling of
  // "older than this" — including one that fetches the newest page forever, or one that drops
  // a message at every page boundary. Two documents per page over a backup of four.
  await withClient(async (client) => {
    const walked = []

    for await (const document of iterDocuments(client, CHAT, { pageSize: 2, max: 50 })) {
      walked.push(document.id)
    }

    assert.ok(walked.length >= 4, `walked only ${walked.length} documents`)
    assert.deepEqual(walked, [...new Set(walked)], 'the walk handed back a message twice')
    assert.deepEqual(walked, [...walked].sort((a, b) => b - a), 'the walk lost its order')
  })
